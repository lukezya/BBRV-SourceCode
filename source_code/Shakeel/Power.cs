...
//Responsible for power question.
public class Power : MonoBehaviour
{
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            if (i == 0)
            {
                ...
                lines[i + 1] += "import inspect, sys\n" +
                    "f = open(\"Assets\\PythonScripts\\PowerFile.txt\", \"w\")\n" +
                    "f.close()";

            }

            if (lines[i].Contains("if y == 0:"))
            {
                lines[i + 1] += "        global out\n" +
                    "        out += str(x) + \",\" + str(y)\n" +
                    "        f = open(\"Assets\\PythonScripts\\PowerFile.txt\", \"a\")\n" +
                    "        f.write(\"\\n\" + out)\n" +
                    "        f.close()";
            }
    ...
    void saveCodeToFile()
    {
        ...
        Debug.Log("Saving to file Power.py");

        System.IO.File.WriteAllText(@"Assets\\PythonScripts\\Power.py", code);

        //Compile
        Instantiate(compileCube, transform.position, Quaternion.identity);

        GameObject.Find("Stack").GetComponent<PowerStackCreator>().start = true;
    }
}
